                 ...
                 gint                  radius)
{
  ...
  size_t global_ws[2];
  cl_mem temp_tex;
  gint stride = 16; /*RaGaBaA float*/
  temp_tex = gegl_clCreateBuffer(gegl_cl_get_context(),CL_MEM_READ_WRITE,
               src_rect->width * src_rect->height * stride, NULL, &cl_err);
  ...
  cl_err |= gegl_clSetKernelArg(cl_data->kernel[0], 1, sizeof(cl_mem),   (void*)&temp_tex);
  ...
  cl_err |= gegl_clSetKernelArg(cl_data->kernel[1], 0, sizeof(cl_mem),   (void*)&temp_tex);
            ...
            const GeglRectangle *result)
{
  ...
  gint read = gegl_buffer_cl_iterator_add (i, input, result, in_format,  GEGL_CL_BUFFER_READ, LAPLACE_RADIUS);
  while (gegl_buffer_cl_iterator_next (i, &err))
  {
    if (err) return FALSE;
    for (j=0; j < i->n; j++)
    {
      cl_err = cl_edge_laplace(i->tex[read][j], i->tex[0][j],&i->roi[read][j], &i->roi[0][j], LAPLACE_RADIUS);
